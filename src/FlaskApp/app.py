...
@app.route("/usersubmitsjournalentry", methods=["POST", "GET"])
def usersubmitsjournalentry(): 
    if(request.method == "POST"): 
        ...
        try: 
            ...
            client.put_object(Body=journal_data, Bucket=s3_bucket, Key="UserJournalEntry-{}-{}.txt".format(journal_title, ct)) 
        ...
        print(url_for('journalentrysubmitted'))
        return redirect(url_for('journalentrysubmitted'),code=200)
...
@app.route("/journalentrysubmitted", methods=["GET", "POST"])
def journalentrysubmitted(): 
    return render_template("UserHome.html") 
